  ...
  <header class="z-100 bg-#f4f6f9  transform-translate-x-0px  sticky top-0px w-100%">
    <div class="topBar  top-4px bg-#fff">
      <div class="bar-inner flex items-center max-w-1156px min-w-1000px  box-border">
        <el-image src="/src/assets/images/author/zhihu.png" fit="cover" class="w-116px h-50px cursor-pointer"></el-image>
        ...
        <div class="bar-userInfo flex items-center m-l-30px">
          ...
          <el-image src="/src/assets/images/people/avator.jpg" fit="cover" class="w-30px h-30px m-l-8px" :lazy="true"></el-image>
...
<div class="left content-card ">
  <el-card shadow="always" :body-style="{ padding: '20px' }">
    <template #header>
     <TabLine :tab-list="ContentBarList" default="recommend" tab-margin="12px" :show-line="false" active-color="#1772f6" @click-tab="(e:string)=>$router.push({path:`/author/${e}`})"/>
    </template>
    <!-- card body -->
...
<div class="right  content-card flex-auto max-w-368px">
  ...
  <div class="flex content-card-inner flex-col">
    <div class="flex justify-center">
      <button class="content-card-button flex flex-col justify-center items-center" v-for="item in iconList" :key="item.label">
...
<div class="cursor-pointer" @click="pageChange(true)">
  ...
  <div v-for="item in categoryList" class="category-item  grid justify-center content-center cursor-pointer" :style="{color:item.color}">
...
{icon:'lets-icons:lightning-duotone',desc:'live',color:'#ffcf00'},
{icon:'gridicons:book',desc:'书店',color:'#43d480'},
{icon:'ion:flower-outline',desc:'圆桌',color:'#0066ff'}, 
{icon:'mage:pen',desc:'专栏',color:'#0f88eb'},
{icon:'ri:money-cny-box-fill',desc:'付费咨询',color:'#5478f0'},
{icon:'simple-icons:bookstack',desc:'百科',color:'#5868d1'}
])
...
.recommend_item :deep(.el-button--primary) ,.content-card-inner :deep(.el-button--primary) {
